...
- **Web search:** `mcp__plugin_search_brave-search__brave_web_search` — use aggressively for recency and primary-source verification. Pass queries in English for best results. Use `count: 5` minimum per claim. Rate limit: 50 req/sec — parallel calls are fine.
- **URL content:** `mcp__plugin_search_firecrawl__firecrawl_scrape` — fetch full page content from any URL (docs, changelogs, release notes). Use `url` (string) and `formats: ["markdown"]` parameters. Use to verify URLs referenced in the artifact.
