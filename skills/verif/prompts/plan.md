...
2. EVIDENCE AUDIT — Read EVERY file listed in the Evidence section. Verify the plan correctly interprets the code — check both that the cited location exists and that it says what the plan claims. Flag any misinterpretation. Claims about schedules, times and frequency ("cron 15:00", "the batch lands ~12:00 UTC", "every day", "once a week") are verified against RUN ARTIFACTS — mtime of the files the run produced, dates in their frontmatter, the registered-task list — never against the plan's own text or a spec line: an Evidence line records what the plan's author read or remembered, not what actually runs (a plan claiming "cron 15:00 Warsaw" was refuted by `stat -f %Sm` on eight notes showing 18:43–18:51 and no registered cron at all). If there is nothing to check it against, that is itself a finding.
