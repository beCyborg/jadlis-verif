---
name: verif
description: "Triple adversarial verification of a plan/research/doc: Codex + Fable + Grok in parallel, arbiter merge, batch interview, fixes. Flags: --only codex|fable|grok, --report-only, --json. Triggers: /verif, verify plan, adversarial review, fact-check. RU triggers: верифицируй план, проверь план, проверь ресерч на факты, фактчек документа. Do NOT use for: code → /code-review; research → /full-research."
...
claude -p "$(cat "{PROMPT_FILE}")" \
  ...
  --allowedTools "Read,Grep,Glob,mcp__plugin_search_brave-search__brave_web_search,mcp__plugin_search_firecrawl__firecrawl_scrape" \
